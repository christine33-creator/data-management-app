...
if ($_SESSION["userType"] == "Player") {
    $playerID = getPlayerID($conn, $_SESSION["Userid"]);
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["teamID"])) {
        ...
        if (isPlayerInTeam($conn, $playerID, $teamID)) {
            ...
            $stmt = $conn->prepare("DELETE FROM PLAYS_IN WHERE PlayerID = ? AND TeamID = ?");
    ...
    // Display the form to leave a team
    echo "<form action=\"PlayerLeaveATeam.php\" method=\"POST\">";
...
function getPlayerID($conn, $userid) {
    ...
    $stmt = $conn->prepare("SELECT PlayerID FROM PLAYER WHERE UserID = ?");
...
function isPlayerInTeam($conn, $playerID, $teamID) {
    // Check if the coach is associated with the team as the head coach
